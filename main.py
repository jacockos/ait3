...
class FlightManager:
    def fly(self, letiště_list, letadlo):

        departure_airport = random.choice(letiště_list)
        destination_airport = random.choice([airport for airport in letiště_list if airport != departure_airport])


        departure_airport.odeber_letadlo(letadlo)


        destination_airport.přidej_letadlo(letadlo)


        flight_info = f"Letadlo {letadlo.zobrazit_letadlo()} letí z {departure_airport.jméno} do {destination_airport.jméno}.\n"
        flight_info += f"Informace o letu: {letadlo.zobrazit_kapitana_a_pasazery()}"


        flight_info += f"\nLetadlo úspěšně doletělo na letiště {destination_airport.jméno}.\n"
...
letiště1.přidej_letadlo(letadlo2)
letiště2.přidej_letadlo(letadlo3)


flight_manager = FlightManager()


print(flight_manager.fly([letiště1, letiště2, letiště3], letadlo1))
